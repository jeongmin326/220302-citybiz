...
<body class="bg-[#F8FAFC] flex flex-col min-h-screen text-slate-800">
    ...
    <main class="flex-grow max-w-6xl mx-auto w-full px-4 sm:px-6 py-12" id="mainContent">
        ...
        <%-- 플랜 정보 카드 --%>
        <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-3xl p-6 border border-purple-200 shadow-sm mb-8">
            <div class="flex items-center justify-between">
                    ...
                    <div>
                        ...
                        <c:choose>
                            <c:when test="${planType == 'MONTHLY'}">
                                <p class="text-2xl font-bold text-purple-900">월간 플랜</p>
                            </c:when>
                            <c:when test="${planType == 'YEARLY'}">
                                <p class="text-2xl font-bold text-purple-900">연간 플랜</p>
                            </c:when>
                            <c:otherwise>
                ...
                <c:if test="${planType == 'MONTHLY' || planType == 'YEARLY'}">
                    <a href="/charge/plan" class="bg-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-purple-700 transition">플랜 갱신</a>
                </c:if>
                <c:if test="${planType == 'FREE' || planType == null}">
                    <a href="/charge/plan" class="bg-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-purple-700 transition">플랜 가입하기</a>
                </c:if>
            </div>
        </div>
